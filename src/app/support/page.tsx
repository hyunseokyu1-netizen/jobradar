...
export const metadata = { title: '고객센터 — MatchDa' }

const FAQS = [
  {
    q: '공고는 어떻게 추가하나요?',
    a: '지원 현황에서 채용공고 URL을 붙여넣거나, "직접 추가"에서 공고 페이지 전체를 복사(Ctrl+A → Ctrl+C)해 붙여넣으면 AI가 자동으로 정리해줍니다. 잡 탐색에서 수집한 공고는 "관리 보내기"로 추가할 수 있어요. Indeed·LinkedIn·Glassdoor 같은 사이트에서 내 관심 직무·지역·키워드로 검색해두고(검색 조건 저장·잡 알림 활용), 마음에 드는 공고의 링크를 복사해 붙여넣으면 여러 잡보드의 공고를 한 곳에 모아 관리할 수 있어요.',
  },
  {
    q: '영어 이력서는 어떻게 만드나요?',
  ...
  {
    q: '구독은 어떻게 해지하나요?',
    a: '요금제 페이지의 "구독 관리" 버튼을 누르면 Stripe 고객 포털에서 언제든 해지할 수 있습니다. 해지해도 현재 결제 주기 종료일까지 프리미엄이 유지됩니다.',
  },
  {
    q: '비밀번호를 바꾸고 싶어요.',
