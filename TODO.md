...
- [x] JD 맞춤 이력서 생성 (원본 이력서 + JD → ATS 친화 맞춤 이력서, 편집/저장/다운로드) — ⚠️ migration 008 수동 적용 필요
- [x] 원본 양식 유지 DOCX 생성 (원본 DOCX 서식 그대로, 문단 텍스트만 JD 맞춤 교체) — ⚠️ migration 009 수동 적용 + 프로필에서 DOCX 재업로드 필요
...
- [x] 드래그 순서 변경 (@dnd-kit)
- [x] 메모 입력 기능
- [x] URL 중복 등록 시 알림창 표시 (이미 등록된 공고 안내, 타 유저 공고는 재스크래핑 없이 내 목록에 추가)
- [x] 상태별 필터링 (전체/관심있음/고민중/지원완료/면접/합격/불합격/패스 칩 필터)
